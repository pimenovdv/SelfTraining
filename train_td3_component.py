...
def main():
    ...
    for ep in range(args.epochs):
        ...
        while not done and steps < 100:
            ...
            if len(replay_buffer) >= args.batch_size:
                ...
                noise = np.random.normal(0, args.policy_noise, size=a_batch.shape)
                noise = np.clip(noise, -args.noise_clip, args.noise_clip)
                na_batch = target_actor.forward(ns_batch) + noise
                na_batch = np.clip(na_batch, -max_action, max_action)
    ...
    report_path = os.path.join(docs_dir, "0129_train_td3_component.md")


    report_content = f'''# Experiment 0129: Train Twin Delayed DDPG (TD3) Component
...
*   **Hyperparameters:** `hidden_dim` = {args.hidden_dim}, `epochs` = {args.epochs}, `lr` = {args.lr}, `gamma` = {args.gamma}, `tau` = {args.tau}, `batch_size` = {args.batch_size}, `buffer_size` = {args.buffer_size}
